...
async def remove_peer(msg):
    ...
    if classes == 'CAR': 
        ...
        if uid in peers_controller:
            ws, raddr, status = peers_controller[uid]
            if status and status != 'session':
                await cleanup_room(uid, status)
...
async def connection_handler(ws, msg):
    global peers_car, peers_controller, sessions, rooms
    raddr = ws.remote_address
    peer_status = None
    classes, hello, uid = msg.split(maxsplit=2)
    if classes == 'CAR':
        peers_car[uid] = [ws, raddr, peer_status]
    else:
        peers_controller[uid] = [ws, raddr, peer_status]
    ...
    while True:
        ...
        if classes == 'CAR':
            peer_status = peers_car[uid][2]
        else:
            peer_status = peers_controller[uid][2]
	...
	##############################################################################################
        ...
        if peer_status is not None:
            ...
            if peer_status == 'session':
                other_id = sessions[uid]
                if classes == 'CAR':
                    wso, oaddr, status = peers_controller[other_id]
                else:
                    wso, oaddr, status = peers_car[other_id]
...
async def hello_peer(ws):
    ...
    if not uid or uid in peers_car or uid in peers_controller or uid.split() != [uid]: # no whitespace
        await ws.close(code=1002, reason='invalid peer uid')
        raise Exception("Invalid uid {!r} from {!r}".format(uid, raddr))
    # Send back a HELLO
    if classes == 'CAR':
        await ws.send('HELLO CAR')
    elif classes == 'CONTROLLER':
        await ws.send('HELLO CONTROLLER')
    else:
